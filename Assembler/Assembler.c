        ...
        if (analyzed_line.analyzed_line_opt == directive)
        {
            ...
            if (analyzed_line.dir_or_inst.directive.dir_opt == dir_entry || analyzed_line.dir_or_inst.directive.dir_opt == dir_extern)
            {
                ...
                if (temp_symbol)
                {
                    ...
                    {
                        assembler_error(line_index, "Extern label redeclared");
                    }
                }
                else
                {
...
Object_File second_move(Object_File object_file)
{
    ...
    while (object_file.code_section != NULL) 
    {
        ...
        {
            if (strlen(object_file.code_section->missing_label[i]) > 0)
            {
                original_current_line = get_compiled_line(object_file.code_section, object_file.code_section->line_index);
                ...
                if (temp_symbol == NULL) 
                {
                    assembler_error(object_file.code_section->line_index, "Can't find the label in the Symbol Table");
                }
                else if (temp_symbol->symbol_opt == symbol_extern_def)
                {
                    extern_calls = insert_symbol_to_table(extern_calls, object_file.code_section->missing_label[i], 
                        object_file.code_section->line_index, symbol_extern_def, &(object_file.code_section->begin_address) + 1);
                }   
                ...
                {
                    assembler_error(object_file.code_section->line_index, "Can't find the address of the label in the Symbol Table");
                }
                original_current_line->words[i + 1] = current_address; /* Index '0' of words is the instruction word. That's why 'i + 1' */
            }
        }
