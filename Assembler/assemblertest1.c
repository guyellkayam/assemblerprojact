...
int count_symbols(Symbol *symbol_table)
{
    int count = 0;
            ...
            if (analyzed_line.dir_or_inst.directive.dir_opt == dir_entry || analyzed_line.dir_or_inst.directive.dir_opt == dir_extern)
            {
                ...
                if (temp_symbol)
                {
                    if (analyzed_line.dir_or_inst.directive.dir_opt == dir_entry)
                    {
                        ...
                        entry_symbol = analyzed_line.dir_or_inst.directive.dir_operand.label_name;
                        printf("Found entry symbol: %s\n", entry_symbol);
                        entry_calls = insert_symbol_to_table(entry_calls, entry_symbol, line_index, symbol_entry_def, NULL);
                    }
        ...
        for (i = 0; i < current_section->num_of_words - 1; i++)
        {
            if (strlen(current_section->missing_label[i]) > 0)
            {
                ...
                if (temp_symbol == NULL) 
                {
                    ...
                    extern_address = current_section->begin_address + i + 1;
                ...
                {
                    printf("Adding external symbol: %s at address %d\n", 
                           current_section->missing_label[i], 
                           current_section->begin_address + i + 1);
                    extern_address = current_section->begin_address + i + 1;
                    extern_calls = insert_symbol_to_table(extern_calls, 
                                                          current_section->missing_label[i], 
                                                          current_section->line_index, 
