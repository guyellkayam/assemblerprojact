        ...
        if (analyzed_line.analyzed_line_opt == directive)
{
        ...
        if (analyzed_line.analyzed_line_opt == instruction) 
        {
            ...
            inst_word = (analyzed_line.dir_or_inst.instruction.inst_opt << 6) & 0xFC0;
            inst_word |= (analyzed_line.dir_or_inst.instruction.inst_operand_options[0] << 4) & 0x030;
            inst_word |= (analyzed_line.dir_or_inst.instruction.inst_operand_options[1] << 2) & 0x00C;
            inst_word |= 0x1; /* Set A bit */
...
Object_File second_move(Object_File object_file)
{
    ...
    unsigned int extern_address;  /* Declare at the beginning of the function */
